...
**Presupuestación**
- **La carga es por planilla** (agregado el 23/09 a pedido de Ricardo). Al elegir un rubro aparecen TODOS los materiales de ese rubro del catálogo, con su unidad, y se completa cantidad y precio en las filas que vayan; las vacías no se cargan. Guardar **reemplaza** los ítems de ese rubro: mandar solo lo que cambió obligaría a llevar la cuenta de qué fila se vació, y un despiste ahí deja ítems que no se ven pero suman al total. El "agregar ítem" de a uno sigue existiendo para lo que no está en el catálogo. **La planilla se despliega en la página, no en un modal** (24/09): una ventana emergente tapa el presupuesto que se está armando, que es lo que uno quiere mirar mientras carga un rubro.
- **El listado entra por la obra, no por el presupuesto** (23/09). `GET /api/presupuestos/por-obra` devuelve una entrada por obra con sus instancias adentro, y **cuál de ellas gobierna la obra**: el definitivo aprobado si existe, y si no, el último del circuito que siga en pie (un rechazado no representa a la obra). Esa elección se decide en el backend y no en la pantalla porque el Tablero usa el mismo número: si cada vista lo resolviera por su cuenta, podrían mostrar totales distintos de la misma obra.
- **Hay UN rubro marcado como mano de obra** (`rubro.es_mano_de_obra`, `V18`). Su planilla no lista materiales sino **los otros rubros**, para cargar de una sola vez cuánto sale la mano de obra de cada especialidad; los ítems que genera pertenecen al rubro Mano de obra, así ese total queda junto. **Se carga por jornales × valor del jornal O escribiendo el total directo** (24/09): a veces se saben los jornales y a veces le pasan un precio cerrado. El total directo se guarda como cantidad 1 con unidad `global`, así no hace falta ninguna columna nueva ni un caso especial al sumar. Es una marca y no el nombre: reconocerlo por cómo se llama se rompería al renombrarlo. Un índice único parcial impide que haya dos.
